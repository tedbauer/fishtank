...
export async function POST(request) {
    try {
        const { householdId, choreName, completedByName, excludeUserId, together } = await request.json();

        if (!householdId || !choreName || !process.env.VAPID_PRIVATE_KEY) {
        ...
        const body = together
            ? `${completedByName} & the household did "${choreName}" together 🤝`
            : `${completedByName} just finished "${choreName}"`;
